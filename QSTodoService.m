...
-(QSTodoService *)init
{
    ...
    if (self)
    {
        ...
        MSClient *client = [MSClient clientWithApplicationURLString:@"https://freshprintsservice.azure-mobile.net/"
                                                     applicationKey:@"baJFydlmYLROPHHmpckTpnXbxKNnIK59"];
...
-(void)completeItemGood:(NSDictionary *)item completion:(QSCompletionWithIndexBlock)completion
{
...
-(void)completeItemBad:(NSDictionary *)item completion:(QSCompletionWithIndexBlock)completion
{
    // Cast the public items property to the mutable type (it was created as mutable)
    NSMutableArray *mutableItems = (NSMutableArray *) items;
    ...
    // Set the item to be complete (we need a mutable copy)
    NSMutableDictionary *mutable = [item mutableCopy];
    [mutable setObject:@"verifyfailed" forKey:@"text"];
